...
async function getAllProducts(route) {
...
document.addEventListener("DOMContentLoaded", () => {
  const macaronsList = document.querySelector(".macarons-list");
  const otherList = document.querySelector(".other-list");
  const mochiList = document.querySelector(".mochi-list");
  const path = window.location.pathname;
...
async function loadProducts(productList, route) {
  const productsData = await getAllProducts(route);
  const markup = markupProducts(productsData);
  productList == null ? void 0 : productList.insertAdjacentHTML("beforeend", markup);
  let productCardBtns = document.querySelectorAll(".product-card-btn");
  addEventToBtn(productCardBtns);
}
function markupProducts(productsData) {
  if (productsData) {
